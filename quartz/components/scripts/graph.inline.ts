...
async function renderRadialGraph(container: string, fullSlug: FullSlug) {
  ...
  const svg = d3
   ...
   .attr("height", height)
   .attr("viewBox", [-width / 2 / scale, -height / 2 / scale, width / scale, height / scale])
   .attr("style", "width: 100%; height: auto; font: 16px sans-serif;");
    ...
    if (node.data.links) {
      ...
      node.data.links.forEach((link) => {
        if (nodes.find((n) => n.data.id == link)) {
          links.push({
            source: node,
            target: nodes.find((n) => n.data.id == link)
          });
        }
      });
    }
  ...
  links.forEach((d) => {
    const aS = d.source.x,
      ...
      aT = d.target.x,
      rT = d.target.y,
      aM = (aS + aT) / 2, // middle angle
      rM = (rS + rT) / 2; // middle radius
    // sx = rS * Math.sin(aS), // cartesian coordinate
    // sy = -rS * Math.cos(aS),
    // tx = rS * Math.sin(aT),
    // ty = -rS * Math.cos(aT),
    // mx = rM * Math.sin(aM),
